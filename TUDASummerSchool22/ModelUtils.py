...
def test(data_source, model, verbose=True):
    ...
    for batch_id, (data, targets) in enumerate(data_iterator):
        ...
        total_loss += cross_entropy(output, targets, reduction='sum').item()  # sum up batch loss
    ...
    if verbose:
        print_timed(f'___Test {name}: Average loss: {total_l:.4f}, Accuracy: {correct}/{dataset_size} ({acc:.4f}%)')
...
def visualize_model_predictions(dataset_to_use, model_to_test, CLASSES, STD_DEV,MEAN, batch_index_to_plot=2, show_labels=True):
    ...
    for x in range(8):
        for y in range(4):
            axs[y,x].imshow(unnormalize_image(data[x*8+y].cpu(), STD_DEV=STD_DEV,MEAN=MEAN))
            title = ''
            if show_labels:
...
def evaluate_model(model_to_evaluate, test_data, backdoor_test_data, name=None, verbose=True):
    ...
    backdoor_accuracy = test(backdoor_test_data, model_to_evaluate, verbose=False)
    if verbose:
        print_timed(f'Performance of {name}: MA={main_task_accuracy:1.2f} BA={backdoor_accuracy:1.2f}')
